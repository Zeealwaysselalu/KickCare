...
function initTransactionForm() {
    ...
    const memberStatus =
        document.getElementById("input-status-member")?.value || "none";
    ...
    function updateSummary() {
        ...
        else if (memberStatus === "silver") discountPercent = 0.1;
        else if (memberStatus === "gold") discountPercent = 0.2;
        ...
        if (displayDiscount)
            displayDiscount.textContent = `- ${formatRupiah(discountAmount)}`;
        if (totalPriceElement)
            totalPriceElement.textContent = formatRupiah(finalPrice);
    ...
    outletRadios.forEach((radio) => {
        radio.addEventListener("change", () => {
            ...
            if (displayOutlet) {
                ...
                displayOutlet.classList.replace(
                    "text-gray-400",
                    "text-blue-400",
                );
            }
...
function initGlobalEvents() {
    document.addEventListener("click", (e) => {
        ...
        // Handle Tombol Status (Detail Pesanan)
