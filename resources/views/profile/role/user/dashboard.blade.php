    ...
    <div class="p-6 lg:p-8 max-w-7xl mx-auto">
        ...
        <div class="grid grid-cols-12 gap-6 items-stretch">
            <div class="col-span-12 lg:col-span-8 animate-up">
                ...
                <div
                    class="bg-gradient-to-br {{ $m['gradient'] }} rounded-[20px] p-8 relative overflow-hidden card-shadow h-full flex flex-col justify-center min-h-[200px]">
                    <div class="relative z-10">
                        <span
                            class="bg-white/20 text-white font-roboto text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest border border-white/20">
                            {{ $m['tier'] }}
                        </span>
                        <h2
                            class="font-montserrat font-extrabold text-[28px] {{ $m['text'] }} mt-3 uppercase italic leading-tight">
                            {{ $m['label'] }}
                        </h2>
                        <p class="font-roboto text-white/90 text-sm mt-1 max-w-xl">{{ $m['desc'] }}</p>
                        <a href="{{ route('benefits') }}"
                            class="mt-5 inline-block bg-white/10 hover:bg-white/20 border border-white/30 text-white font-roboto font-medium px-6 py-2 rounded-full text-xs transition backdrop-blur-sm shadow-inner">
                            Lihat Benefit
                        </a>
                    </div>
                ...
                <div class="bg-white border border-gray-100 rounded-[20px] p-6 card-shadow h-full flex flex-col">
                    ...
                    @php
                        ...
                        $statusMap = [
                            'pending' => [
                                ...
                                'desc' => 'Menunggu antrean.',
                                'color' => 'blue',
                            ],
                            'sorting' => [
                                'label' => 'Pengecekan',
                                'desc' => 'Memeriksa kondisi sepatu.',
                                'color' => 'blue',
                            ],
                            'washing' => [
                                'label' => 'Sedang Dicuci',
                                'desc' => 'Pembersihan intensif.',
                                'color' => 'blue',
                            ],
                            'drying' => [
                                'label' => 'Pengeringan',
                                'desc' => 'Proses pengeringan aman.',
                                'color' => 'blue',
                            ],
                            'ready' => [
                                'label' => 'Siap Diambil',
                                'desc' => 'Sudah bersih & wangi!',
                                'color' => 'green',
                            ],
                        ];
                    ...
                    <div class="flex-1 flex flex-col justify-center">
                        @if ($latest && $detail && $mainStatus !== 'cancelled')
                            <div class="mb-4">
                                <p class="text-[11px] font-bold text-gray-400 uppercase tracking-wider">Sedang Diproses
                                </p>
                                <h4 class="text-sm font-bold text-gray-800 truncate">{{ $item->shoes_name ?? 'Sepatu' }}
                                </h4>
                            </div>
                            ...
                            <div class="p-4 relative">
                                <div class="flex items-center gap-4">
                                    <div class="rounded-xl flex items-center justify-center text-white">
                                        ...
                                        @else
                                            <div
                                                class="mt-1.5 w-[24px] h-[24px] rounded-full border-4 border-white shadow-sm flex items-center justify-center z-10 bg-blue-500 ring-4 ring-blue-50">
                                            </div>
                                        @endif
                                    </div>
                                    <div>
                                        <h5
                                            class="text-sm font-extrabold text-{{ $current['color'] }}-700 uppercase tracking-tight">
                                            {{ $current['label'] }}</h5>
                                        <p class="text-[10px] text-{{ $current['color'] }}-600 leading-tight mt-0.5">
                                            {{ $current['desc'] }}</p>
                                    </div>
                                </div>
                            </div>

                            <button data-id="{{ $latest->id }}"
                                class="btn-status mt-4 block w-full text-center py-2.5 text-[11px] font-bold text-blue-600 bg-blue-50 rounded-xl hover:bg-blue-100 transition-all uppercase tracking-wide">
                                Detail Pesanan
                            </button>
            ...
            <div class="col-span-12 mt-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                    @php
                        $services = [
                            [
                                ...
                                'label' => 'Deep Clean',
                                'price' => '65k',
                                'desc' => 'Pembersihan menyeluruh luar & dalam.',
                                'color' => 'blue',
                                'icon' =>
                            ...
                            [
                                ...
                                'label' => 'Unyellowing',
                                'price' => '100k',
                                'desc' => 'Menghilangkan noda kuning di midsole.',
                                'color' => 'yellow',
                                'icon' =>
                                    'M12 3v1m0 16v1m9-9h-1M4 12H3m15.364-6.364l-.707.707M6.343 17.657l-.707.707m12.728 0l-.707-.707M6.343 6.364l-.707-.707M15 12a3 3 0 11-6 0 3 3 0 016 0z',
                            ],
                            [
                                'key' => 'repaint',
                                'label' => 'Repaint',
                                'price' => '150k',
                                'desc' => 'Kembalikan warna sepatu seperti baru.',
                                'color' => 'purple',
                                'icon' =>
                                    'M7 21a4 4 0 01-4-4V5a2 2 0 012-2h4a2 2 0 012 2v12a4 4 0 01-4 4zm0 0h12a2 2 0 002-2v-4a2 2 0 00-2-2h-2.343M11 7.343l1.172-1.172a4 4 0 115.656 5.656L16.657 13',
                            ],
                        ];
                    @endphp
                    ...
                    @foreach ($services as $s)
                        <div
                            class="bg-white p-5 rounded-[18px] card-shadow border border-transparent hover:border-blue-500 transition-all group cursor-pointer">
                            <div
                                class="w-12 h-12 bg-{{ $s['color'] }}-50 rounded-xl flex items-center justify-center text-{{ $s['color'] }}-500 mb-4 group-hover:bg-blue-500 group-hover:text-white transition-all">
                            ...
                            <div class="mt-4 flex items-center justify-between border-t border-gray-50 pt-4">
                                ...
                                <a href="{{ route('transactions.create', ['service' => $s['key']]) }}"
                                    class="bg-gray-50 text-gray-800 text-[11px] font-bold px-4 py-2 rounded-lg hover:bg-blue-600 hover:text-white transition-all">
                                    Pilih
                                </a>
                            </div>
                        </div>
            ...
            <div class="col-span-12 mt-8">
                <div class="flex items-center justify-between mb-5">
                    ...
                    <a href="{{ route('pesanan') }}" class="text-blue-600 text-xs font-bold hover:underline">Lihat
                        Semua</a>
                </div>
                                    ...
                                    <tr class="hover:bg-blue-50/20 transition-all group">
                                        <td class="px-6 py-5">
                                            <div class="flex flex-col">
                                                <span
                                                    class="text-[14px] font-bold text-gray-800 group-hover:text-blue-600 transition-colors italic">
                                                    {{ $item->shoes_name ?? '-' }}
                                                </span>
                                                <div class="flex items-center gap-2 mt-1">
                                                    <span
                                                        class="text-[9px] bg-blue-50 text-blue-500 px-2 py-0.5 rounded font-bold uppercase tracking-tighter">
                                                        {{ $item->service ?? '-' }}
                                                    </span>
                                                    <span
                                                        class="text-[9px] text-gray-400 font-mono italic">#{{ $transaction->transaction_code }}</span>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-6 py-5 whitespace-nowrap">
                                            @if ($status === 'pending')
                                                <span
                                                    ...
                                                    <span
                                                        class="w-1.5 h-1.5 bg-blue-500 rounded-full animate-pulse"></span>
                                                    Sedang Dicuci
                                                </span>
                                            ...
                                            @else
                                                <span
                                                    class="inline-flex items-center bg-red-50 text-red-600 text-[10px] px-3 py-1 rounded-full font-bold uppercase border border-red-100">Dibatalkan</span>
                                            @endif
                                        </td>
                                        <td
                                            class="px-6 py-5 text-[14px] font-bold text-gray-800 whitespace-nowrap text-right">
                                        ...
                                        <td class="px-6 py-5">
                                            <div class="flex items-center justify-end gap-2">
                                                ...
                                                @if ($status === 'pending')
                                                    <button
                                                        data-action="{{ route('transactions.cancel', $transaction->id) }}"
                                                        class="btn-cancel w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50 transition-all border border-gray-100">
                                                        <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                            viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                                stroke-width="2"
